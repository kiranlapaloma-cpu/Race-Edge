...
# (Drop-in replacement with Adaptive F-windows + Weight-normalisation)
...
def build_metrics_and_shape(
    ...
    def _adaptive_f_window_cols(D, step, markers):
        ...
        Returns (f_cols, f_dist) according to rules:
          100 m splits:
             ...
             - if D ends with 50: F150 = [D-50, D-150] (2 splits, 150 m)
          200 m splits:
    ...
    def _adaptive_tssp_start(D, step):
        # tsSPI start:
        #   100 m splits: D-300, but if D ends in 50 → D-150
        #   200 m splits: use first span heuristic (100/160/250) else D-400
        if step == 100:
    ...
    if use_weight:
        ...
        def _adj(col):
            ...
            mult = 1.0 + float(weight_sens_per_kg) * PHASE_MULT[col] * (float(weight_baseline) - kg_vec)
...
try:
    ...
    # ---------- EARLY/LATE composite indices (requested) ----------
    # EARLY = 0.6·F + 0.4·tsSPI  (effective columns)
    # LATE  = 0.5·Accel + 0.5·Grind (CG if ON)
    ...
    # ---------- Final rounding ----------
    for c in ["F200_eff","tsSPI_eff","Accel_eff","Grind_eff",      # effective (may be weight-adjusted)
    ...
    w.attrs["GR_COL"] = GR_COL                         # which grind column is used downstream
...
if split_step == 200:
    st.caption("First panel & F-window adapt to odd 200m distances (e.g., 1160→F160, 1450→F250, 1300/1500→F100). Finish column is 200→0.")

# ======================= Sectional Metrics table =====================
st.markdown("## Sectional Metrics (PI v3.2 & GCI + CG + Race Shape)")
show_cols = [
    "Horse", "Finish_Pos", "RaceTime_s",
    "F200_eff", "tsSPI_eff", "Accel_eff",           # effective (weight-aware)
    "Grind_eff", "Grind_CG",
    "EARLY_idx", "LATE_idx",
    "GrindAdjPts", "DeltaG",
    "PI", "PI_RS", "GCI", "GCI_RS"
...
st.caption(
    ...
    f"Weight adj={'ON' if 'USE_WEIGHT' in globals() and USE_WEIGHT else 'OFF'} (baseline={globals().get('WEIGHT_BASELINE',60.0)} kg)."
)
# ======================= End of Batch 2 =======================
