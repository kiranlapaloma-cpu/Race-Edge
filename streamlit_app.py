...
# ======================= Velocity Signature + VES =======================
st.markdown("## Velocity Signature — Top Speed & Sustain")

# Make a soft flag so the plot can auto-activate once Top Speed & Sustain has run
if "Vmax (m/s)" in metrics.columns:
    metrics.attrs["velocity_ready"] = True

# --- required column names (from your Top Speed & Sustain table)
need_cols_vs = ["Horse", "Vmax (m/s)", "Sustain (s>=95%)", "Sustain (m>=95%)"]
has_vs = all(c in metrics.columns for c in need_cols_vs)

if not metrics.attrs.get("velocity_ready", False) or not has_vs:
    st.info("Velocity data not available — run the **Top Speed & Sustain** module first "
            "(needs: `Vmax (m/s)`, `Sustain (s>=95%)`, `Sustain (m>=95%)`).")
else:
    GR_COL = metrics.attrs.get("GR_COL", "Grind")
    D_m = float(race_distance_input)
    ...
    # Bring in optional contextual columns if they exist
    opt_cols = ["VProfile", "TSI", "SSI", "RaceTime_s"]
    for c in (need_cols_vs + opt_cols):
        if c not in VS.columns:
            VS[c] = np.nan

    # Convert to numeric safely
    num_cols = ["Vmax (m/s)", "Sustain (s>=95%)", "Sustain (m>=95%)", "VProfile", "TSI", "SSI", "RaceTime_s"]
    for c in num_cols:
        VS[c] = pd.to_numeric(VS[c], errors="coerce")

    # --- robust baselines
    # Use trimmed field medians if per-horse RaceTime_s missing
    rt_field = VS["RaceTime_s"]
    if not np.isfinite(rt_field).any():
        # Fall back: infer from Accel/Grind indices if available (very mild)
        rt_field = pd.Series(np.nan, index=VS.index)
    race_time_ref = np.nanmedian(rt_field) if np.isfinite(rt_field).any() else np.nan

    # If we still don't have a time reference, normalize by max observed sustain seconds
    # (keeps VES stable even without RaceTime_s in the CSV)
    sustain_s = VS["Sustain (s>=95%)"]
    sustain_m = VS["Sustain (m>=95%)"]

    # --- normalized components (0..1)
    # Seconds near Vmax relative to race time (or to max seen, as a fallback)
    if np.isfinite(race_time_ref) and race_time_ref > 0:
        s_norm = (sustain_s / race_time_ref).clip(lower=0.0, upper=1.0)
    else:
        s_norm = (sustain_s / (np.nanmax(sustain_s) if np.isfinite(np.nanmax(sustain_s)) and np.nanmax(sustain_s) > 0 else 1.0)) \
                  .clip(lower=0.0, upper=1.0)

    # Meters near Vmax relative to race distance (uses UI race distance)
    m_norm = (sustain_m / max(D_m, 1.0)).clip(lower=0.0, upper=1.0)

    # VProfile as a mild efficiency multiplier (centred around ≈6)
    vprof = VS["VProfile"]
    vprof_mult = (vprof / 6.0).clip(lower=0.85, upper=1.15)  # keep it gentle
    vprof_mult = vprof_mult.fillna(1.00)

    # --- Velocity Efficiency Score (0..100)
    # Weight seconds a touch more than meters (both already 0..1)
    raw_eff = 0.60 * s_norm.fillna(0.0) + 0.40 * m_norm.fillna(0.0)
    VS["VES"] = (100.0 * raw_eff * vprof_mult).clip(0.0, 100.0)

    # --- tiering
    def _ves_tier(v):
        if not np.isfinite(v): return ""
        if v >= 95:  return "🏆 Ultra-Efficient"
        if v >= 90:  return "🔥 Highly Efficient"
        if v >= 80:  return "✅ Efficient"
        if v >= 70:  return "🟡 Solid"
        return "🔧 Work-in-Progress"

    VS["Tier"] = VS["VES"].map(_ves_tier)

    # --- presentation table
    show_cols = ["Horse", "VProfile", "TSI", "SSI", "Vmax (m/s)", "Sustain (s>=95%)", "Sustain (m>=95%)", "VES", "Tier"]
    VS_view = VS.loc[:, show_cols].copy()

    # Safety: replace NaN with empty/0 where appropriate to avoid any front-end JSON hiccups
    fmt_cols_1dp = ["VProfile", "TSI", "SSI", "Vmax (m/s)"]
    for c in fmt_cols_1dp:
        VS_view[c] = pd.to_numeric(VS_view[c], errors="coerce").round(1)

    VS_view["Sustain (s>=95%)"] = pd.to_numeric(VS_view["Sustain (s>=95%)"], errors="coerce").round(3)
    VS_view["Sustain (m>=95%)"] = pd.to_numeric(VS_view["Sustain (m>=95%)"], errors="coerce").round(3)
    VS_view["VES"] = pd.to_numeric(VS_view["VES"], errors="coerce").round(1)

    VS_view = VS_view.sort_values(["VES", "Vmax (m/s)", "Sustain (s>=95%)"], ascending=[False, False, False]).reset_index(drop=True)

    st.dataframe(
        VS_view.style.format({
            "VProfile": "{:.2f}",
            "TSI": "{:.1f}",
            "SSI": "{:.1f}",
            "Vmax (m/s)": "{:.3f}",
    ...
    # --- optional mini bar-plot for top 10 by VES (safe Matplotlib)
    try:
        topN = VS_view.head(10).iloc[::-1]  # reverse for horizontal bars
        fig, ax = plt.subplots(figsize=(7.4, 4.8))
        ax.barh(topN["Horse"], topN["VES"], height=0.6)
        for i, v in enumerate(topN["VES"]):
            ax.text(v + 1.0, i, f"{v:.1f}", va="center", fontsize=8)
        ax.set_xlim(0, 100)
        ax.set_xlabel("VES (0–100) →")
        ax.set_title("Velocity Efficiency — Top 10 (time & distance near ≥95% of Vmax)")
    ...
    # --- footnote
    if np.isfinite(race_time_ref):
        st.caption(
            f"**VES** blends time and distance spent at ≥95% of top speed (field-robust), "
            f"lightly modulated by VProfile. Normalised by race time ≈ {race_time_ref:.2f}s "
            f"and race distance {int(D_m)}m. Tiers: 🏆 ≥95, 🔥 ≥90, ✅ ≥80, 🟡 ≥70."
        )
    else:
        st.caption(
            f"**VES** blends time and distance spent at ≥95% of top speed (field-robust), "
            f"lightly modulated by VProfile. Race-time missing → seconds normalised by field maxima. "
            f"Tiers: 🏆 ≥95, 🔥 ≥90, ✅ ≥80, 🟡 ≥70."
        )
# ======================= /Velocity Signature + VES =======================
